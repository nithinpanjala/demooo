...
import com.stg.demo.repository.CartRepository;
...
public class CartServicesImpl implements CartServices {
	...
	public Cart addDish(int cartNo ,Dish dish ,int quantity) {
		...
		cart.getDishes().put(dish.getDishId(), quantity);
	...
	public Cart updateDishQuantity(int cartNo ,Dish dish, int quantity) {
		Cart cart = cartRepository.getById(cartNo);
		cart.getDishes().replace(dish.getDishId(), quantity);
		return cart;
	}
	...
	public Float getTotalPrice(int cartNo ,Cart cart) {
		...
		for (Map.Entry<Integer, Integer> iterator : cart1.getDishes().entrySet()) {
			Dish dish = dishRepository.getById(iterator.getKey());
	        totalPrice +=  dish.getDishPrice() * iterator.getValue();
	    }
		return totalPrice;
	}
